...
## 2026-06-15 — AUTOPILOT
AUTOPILOT: shipped nothing — judge chose nothing: I verified every proposal against the live paper_account.json (700 settles) and the actual bot.py logic. None clears the bar; the honest action is to ship NOTHING.

P1 (tighten brain_adjust gate 0.45->0.70, bot.py:5547): The cohort split is real but the entire dollar signal is ONE outlier. Reproduced exactly: weak cohort (adj<0.70) n=12, -$2.66; strong cohort (adj>=0.70) n=9, +$0.27. But -$2.54 of the -$2.66 is a single trade (Israel-airspace, adj=0.69, sitting right on the proposed boundary). Strip that one outlier and the weak cohort is -$0.12 over 11 trades — flat. The remaining weak cohort is almost entirely weather markets at ~$0.06 stakes (negligible impact). The 'strong' cohort is 8/9 trades at adj=1.0, i.e. the brain ABSTAINED — so the real split is 'brain silent' vs 'brain tilted at all,' on n=21. Also note brain_adjust returns a sizing multiplier bounded [0.4,1.6] (line 2926), not a probability; the proposal mis-describes 0.70 as a confidence threshold. Evidence too thin and outlier-driven.

P2 (jump explorer promotion gap to band 94c, bot.py:5236): I confirmed the factual core is TRUE — with the code's wilson_lower(z=1.0), band 94c (32/32) has lower bound 0.9697 > 0.95 breakeven, meets n>=25 and >=15 families, and is currently blocked because the loop stops at the unproven 95c gap. BUT the proposed change (new_min = max(proven)) promotes the entire 94-96c RANGE onto the main book, including band 95c, which is a measured money-LOSER: 39/42 (92.9%) win rate against a 96% breakeven, -$7.76 over 42 settles (-$0.18/trade). The contiguity guard is not a mere structural quirk — it prevents sizing up a range that contains a sub-breakeven band. The proposal cites only 94c's +$5.63 and ignores that its own change opens the adjacent losing 95c band to quarter-Kelly main-book sizing. Expected value is plausibly negative, not the claimed +$6-8/month. Confidence 0.94 is badly miscalibrated, and weakening a coherence guard to leapfrog a losing band is exactly the kind of 'unblock by deleting the guard' move the hard rules forbid.

P3 (persist DEAD_TOKENS to disk, bot.py:146-180): The proposal itself flags the failure mode — get_json() returns None after exhausting retries on a TRANSIENT network error, so a live-but-flaky token gets persisted and never retried for up to 30 days. The current in-memory cache self-heals on the frequent restarts the proposal cites; persisting it removes that self-healing and converts transient flakiness into a multi-day blind spot on tradeable markets. The benefit (~27s API budget/restart -> speculative '+$1-2/week arb edges') is an unmeasured chain with no link to realized PnL. Trades real correctness risk for speculative efficiency.

P4 (add entry_timestamp to settled record, bot.py:5977): Genuinely low-risk data capture, but the '+$7.94 reclaimed' is self-admittedly hypothetical and the proposal concedes these trades are invisible to dead_cohort() (lane != 'r90'), so it reclaims nothing operationally. The field is consumed nowhere today. Adding an unread field justified by a hypothetical reclaim is activity, not evidence — fails 'ship NOTHING when evidence is weak/absent.'

Per the rules: live settled money outranks model enthusiasm, blast radius must be proportional to evidence, and weak/absent evidence means ship nothing. That is the correct call this cycle.

2026-06-15 AUTOPILOT: shipped nothing — judge chose nothing: Ship NOTHING. All four proposals fail verification against the live data in /Users/you/polymarket-bot.

P1 (high_prob band ROI gate): Evidence is false and overfit to a single trade. Scout claimed '17 trades in 95-98c lost $2.27 (avg -$0.126).' Actual high_prob settled = 21 trades total; the ENTIRE 95-98c loss is one idiosyncratic Politics trade at ep=0.983 (-$2.54). The other 8 trades in the 98c band are net +$0.13, 97c (8 trades) is flat at -$0.016/trade, and 95c is positive. The 'seed oversizes these bands 3-10x' thesis is one bad trade masquerading as a systematic band-edge defect over a 21-trade sample. Reject: weak evidence, overfit to a single outcome.

P2 (clip isotonic to 0.952 in ml.py): Highest blast radius of the four — apply_isotonic() runs on EVERY entry decision via brain_adjust — for a self-estimated ~$0.89-$1.19 effect on the settled corpus. The specific calibration plateau it describes (25/29 points Y=1.0, x in [0.92716, 0.9365]) is not present in the live market_model.json, so the central evidence is unverifiable. It also conflates the selection-biased market-price win rate (entry>=0.92 -> 95.22%, which I confirmed) with the model's calibration target, and the benefit mechanism is self-contradictory (clipping lowers p_model, which shrinks estimated edge). Large surface, muddled mechanism, tiny/unverifiable payoff. Reject.

P3 (exclude crypto explore 95c+): Largest claimed PnL (-$24.88 over 187 trades) but statistically hollow and factually wrong on specifics. Scout claimed '7 losses avg -$5.51 each'; actual = exactly 4 losses, each a full-position wipeout (~$9.50-$9.80 cost), and all 4 closed on a SINGLE day (2026-06-15) during one adverse BTC move through the 66k/67k levels. They are one correlated regime event, not 4 independent samples (95% CI on loss count ~ [0, 8]; effective independent tail events ~1-2). The 183 winning trades each pay only +$0.075 (structural 95c+ payoff). The true issue is correlated tail risk / sizing on rare crypto resolutions — already directly addressed by the shipped 'crypto per-resolution-hour correlation cap' (completed task #1). A blanket 95c+ crypto ban is cruder, redundant with existing machinery, and would kill 183 winners to dodge a one-day tail. Blast radius out of proportion to one-event evidence. Reject.

P4 (filter stale h<0 rows in snapshot_once, bot.py ~1108): Real and verifiable in code, but the scout states 'Expected PnL impact: ZERO' — it is pipeline housekeeping (training already filters h<1 downstream). No edge, plus a non-trivial correctness risk: the recorder/CORPUS feeds the outcome-labeling pipeline, and dropping already-resolved (h<0) market rows at the entry point could starve resolution-label generation. Zero upside against an unverified downside is not a good use of the one-change-per-cycle budget. Reject.

Per the hard rules: evidence is weak, false, or absent across the board; the honest default is to ship nothing this cycle.

2026-06-15 AUTOPILOT: shipped nothing — judge chose nothing: Ship NOTHING. I verified each proposal against the live data (paper_account.json: 721 settles) and the code, and none clears the bar of strong measured evidence with impact proportional to blast radius.

The two highest-confidence proposals both fail on inspection:

- Isotonic cap at 0.95 (conf 0.88): The data claim is true — brain.json's cal maps every p>=0.93 to exactly 1.0 (real tail overconfidence). But the proposal's CAUSAL claim is false. kelly_dollars() (bot.py:1356) sizes high_prob from wilson_lower() on empirical band stats and NEVER reads the cal table. The cal only feeds brain_adjust(), which returns a multiplier hard-bounded to [0.4, 1.6]. I computed the real effect: clipping to 0.95 reduces the brain tilt on 0.96-0.99 entries from ~1.03x to ~0.99x — about 4.7%. It cannot 'prevent 100% bankroll Kelly allocation' (the multiplier is bounded; that pathology is impossible), and the '$5-15/month' / '163 LogLoss units' figures are misattributed. Wrong line numbers too. Evidence rests on a false mechanism.

- chartml gate 0.5->0.60 for daytrade (conf 0.92): daytrade allocation is $0 (config + learning.json). Strategy is dark; zero impact until re-allocated. Cosmetic.

The most genuinely measured proposal, the kelly_fraction 0.4->0.25 revert (conf 0.72), is reacting to noise. The entire cited -$2.39 high_prob loss is one geopolitical tail event: 'Israel closes its airspace by June 15?' settled -$2.54 on a $118.94 position. That single trade exceeds the band's whole net loss; the other 98c loss is -$0.24 and the rest of the book (97c -$0.13, 99c -$0.12) is flat. With n=21 and the 98c band at 7 wins / 2 losses, 2 losses where ~1 is expected is well within noise. Worse, that $118.94 position BREACHED the $100 max_dollars cap — a bug already fixed in HEAD (commit 66dc5bb, 'Fix max_dollars cap breach in Kelly sizing'). So the one loss driving the whole signal came from an already-fixed sizing bug, not from kelly_fraction itself. Reverting on this basis would be chasing a single adverse resolution.

The remaining three are current no-ops by their own admission: news gate (news alloc=$0, realized -$4.07, so the existing budget<cost gate already blocks all new entries — the change affects only past trades); resolution-timestamp capture (pure schema/observability, no PnL thesis); and the p0 dead-cohort filter (proposal states p0=0.874 both ways, 1 r90 trade which is Crypto not Sports — and this is the exact DEFERRED-hold item in memory, on hold until dead-cohort sports accrue, still 0).

high_prob is dead-cohort clean (0 r90/sports kelly trades in the 21), so no double-counting issue — but a clean -$2.39 dominated by one fixed-bug tail trade is not evidence to size against. One change per cycle, statistical honesty over activity: the correct action is to ship nothing.
2026-06-15: AUTOPILOT: shipped nothing — adversarial review killed it: failed review (1/3 cleared). commit 40caa73
2026-06-15: AUTOPILOT: shipped nothing — judge chose nothing: No proposal clears the bar; the honest action this cycle is to ship NOTHING.

Proposal 1 (Kelly VWAP oversizing fix) — REJECTED as mechanically false. Its premise that 'VWAP fill improves the price' (fill < ask) is impossible: vwap_fill (bot.py:509) walks the book from the best ask into progressively WORSE levels, so the returned VWAP is always >= the top ask. When line 5562 sets price=fill, price can only rise or stay equal, never fall, so it cannot cause oversizing — recomputing shares at the fill would shrink size, not grow it. The impact gate at line 5559 already rejects any fill more than 0.005 above ask, capping the worst-case cost overshoot at ~0.2%, not the claimed 45% (which would require fill = 0.45*ask). For the high_prob/kelly path, lines 5689-5700 ALREADY recompute new_shares=int(dollars/price) at the post-fill price and re-validate depth. The cited 121-shares example and '$71.52 oversizing' cannot be produced by this code. Confidence 0.95 was misplaced.

Proposal 3 (exclude explore 95c+) — REJECTED: misdiagnosis + over-broad blast radius. The explore >=95c band is negative only because of 4 catastrophic Bitcoin price-move blowouts (-$9.48 to -$9.79, summing -$38.56); the other 167 trades in the 95-98c band are +$12.14, and the 99c band is itself positive (+$1.54). A blanket price gate would kill that positive 99c band plus +$12 of healthy non-crypto fills to 'fix' what is really crypto tail risk — already the domain of existing crypto-specific controls (the per-resolution-hour correlation cap was already shipped). Price band is the wrong lever.

Proposal 2 (rebalance 4.5x explore, refund news+daytrade) — REJECTED: explore's measured edge is breakeven, not the implied 90% alpha. +$2.08 over 685 settles (+$0.003/trade), and the entire margin is hostage to crypto tails (catastrophic <-$5 trades sum -$76.14; ex-tail it is +$78). 90.4% WR is calibration, not EV. Worse, it re-funds news ($0->$250, -$4.07, correctly shelved) and daytrade ($0->$250, -$12.89, the in-game sports trap, correctly shelved) — reopening a known-losing path is activity for its own sake and brushes the sports trap.

Proposal 4 (research() in evolver_loop) — REJECTED: weak evidence. The 'regime shift' rests on n=3 (99c) and n=8 (97c) fresh settles, and the +$40-75/month is speculative recalibration, not measured PnL. It also injects a recurring 25-30s blocking network call with unquantified failure modes for an unproven gain.

Proposal 5 (dead_cohort filter in brain_online_learn) — closest to legitimate: the asymmetry is real (brain_train at line 2397 skips dead_cohort(t); brain_online_learn at 2937 skips only arbitrage). But the scout itself states current live impact is ZERO, and I confirmed 0 dead_cohort settled trades (and only 1 r90-lane settle) in paper_account.json. It is pure future-proofing against a config-reversible sports-ban reversion that hasn't happened, while touching the global brain weights/drift/AdaGrad path. Per the rules, when evidence is absent the honest action is to ship nothing; a zero-impact change to the learning core is not this cycle's one change.

Verified files: /Users/you/polymarket-bot/bot.py (lines 509-520 vwap_fill, 5500-5563 sizing, 5675-5713 kelly share recompute, 5784-5786 position cost, 759-768 dead_cohort, 2390-2398 brain_train filter, 2929-2957 brain_online_learn), /Users/you/polymarket-bot/paper_account.json (737 settles), /Users/you/polymarket-bot/research_results.json (mtime 2026-06-12).

## 2026-06-15
AUTOPILOT: shipped nothing — judge chose nothing: Ship nothing. I verified all four proposals against live code (bot.py at HEAD 54a0fa4, clean tree) and live data (paper_account.json, config.json, QUANT_LOG.md, owner memory). None clears the bar.

PROPOSAL #2 (position cap) is the only one with real, severe measured evidence — and it still fails as written. The -$1,090.97 loss on 2026-06-16 is REAL and confirmed: 1,387 explore settles, of which 1,355 are floor_fill positions in category 'Other' totaling -$1,090.93; account is now $752 cash / -$1,073 realized; explore holds 5,586 open positions vs a 600 cap. BUT the breach comes entirely from maintain_trade_floor() (bot.py:6281), which appends directly to account['positions'] in a cash-only loop and NEVER calls open_position(). The proposal prescribes adding the cap check inside open_position() at ~line 6050 — a code path the catastrophe does not flow through. The fix as written would not have prevented the cited event, and its causal story (a '5.4x velocity spike' / '799 entries in one hour') misattributes the cause. The regular explore scanner (scan_high_prob, bot.py:5700) ALREADY enforces max_open_positions via room = cap - open_count. Capping the floor also brushes against owner do-NOT-revert no-cap policy, so it needs careful, correctly-targeted implementation in the floor itself plus likely owner sign-off — not a misfire in the wrong function. A real problem deserving a correct proposal next cycle, not this one.

PROPOSAL #4 (dead_cohort Sports) fails era hygiene in reverse. The 16 Sports trades (3 daytrade, 13 explore, -$15.28, 25% WR) are confirmed in the data, all lane=None, all closed 06-13+. But they entered via REGULAR explore/daytrade — entry modes the current code CAN still produce. The only structural Sports exclusion is the r90/kelly lane (dead_cohort's existing match) plus the live in-game ban (is_in_game); per owner never-block policy Sports is downsized to 0.25, NOT banned. So these are live, repeatable evidence, not dead cohort. Removing 16 losers under a false 'banned mode' premise is exactly the cherry-picking dead_cohort's own docstring warns against ('wins and losses alike, or the filter is cherry-picking') — and it would make the specialist LESS pessimistic about a genuinely losing live cohort. Reject.

PROPOSAL #1 (Platt/isotonic override) overrides an already-honest held-out-logloss race (bot.py:2697-2699) with a hand-tuned |a|>1.5 heuristic, justified by per-band miscalibration across only ~21 total high-prob settles (a handful per 97/98/99c band) — far below the codebase's own n>=8-material / n>=15-25 promotion bars. Model selection by enthusiasm on tiny n. Reject.

PROPOSAL #3 (stale-market filter) rests on fabricated/incoherent evidence: it cites paper_account.json for CSV snapshot data (snapshots write to DATA_DIR/snapshots-*.csv); its numbers self-contradict (7.8 GB/day vs 234 MB/month); snapshot_once already passes closed=false and end_date_min=now so the API already excludes h<0 markets; and skipping a w.writerow frees ZERO API budget, killing the proposal's entire 'reinvest freed budget' rationale. Reject.

Shipping nothing is the honest default. No safety rail touched, no dead-cohort double-count, no activity-for-its-own-sake change introduced.

2026-06-15 — AUTOPILOT: shipped nothing — judge chose nothing: Ship nothing. All three proposals fail verification against live data at ~/polymarket-bot.

Proposal #2 (disable floor_edge_gate) rests on a fabricated diagnosis. `floor_fill: True` is set in exactly ONE place (bot.py:6355), AFTER the price-band check at bot.py:6328, so every floor_fill entry necessarily passed 0.75<=price<=0.92. The band gate deployed only ~2h ago (commit 160eba8, 2026-06-15 21:33 PDT) and has produced ZERO new entries — all 5,504 dated floor positions were opened pre-deploy under the old buy-everything policy. The proposal's '112 post-deployment trades, 90% outside the band' are 127 trades that merely SETTLED after deploy (the `closed` field) but were OPENED before the gate existed; the scout confused settle-time with entry-time. 91% out-of-band entries are arithmetically impossible under the live gate, so the gate is not broken — there is simply no post-deploy evidence to judge it yet. Disabling a freshly-deployed gate on phantom evidence is activity for its own sake.

Proposal #3 (add dead_cohort guard to brain_online_learn) is a no-op built on fabricated numbers and inverts era hygiene. The code claim is accurate (bot.py:3067 checks only arbitrage; brain_train at 2527 also filters dead_cohort), but actual dead_cohort settles = 0 and total Sports-category settles = 16 (all lane=None, -$15.28), not the claimed '287 trades / -$190.09 / 1.7% WR.' Adding the guard filters nothing today. Worse, the proposal's stated INTENT is to filter lane=None sports — which are NOT dead_cohort (dead_cohort requires lane=='r90'). Those are live, repeatable losers that legitimately must teach the model; filtering them is exactly the cherry-picking the dead_cohort docstring (bot.py:887) warns against. This both rests on absent evidence and would weaken era-hygiene honesty.

Proposal #1 (brain gate 0.45->0.55) has accurate code locations (bot.py:5888/6846/7002) but only modeled, weak evidence (self-rated confidence 0.58). Every PnL figure is an estimate; there is zero live-settled evidence that 0.55 outperforms 0.45. It is internally contradictory — it cites high brain credibility yet admits the model is underconfident in 4/6 calibration bins, meaning a tighter entry gate would reject entries the model wrongly scored low. It changes a gate shared by high_prob/news/daytrade/explore at once on speculation. Per the rule that live settled money outranks every model/backtest and that weak/absent evidence means ship NOTHING, it does not clear the bar.

Baseline integrity confirmed: 14/14 tests pass on current HEAD. Honest default is to ship nothing this cycle.

## 2026-06-15 — AUTOPILOT: shipped nothing — judge chose nothing: All five proposals fail verification; the honest action is to ship nothing.

P1 (brain_adjust adj>1.0 gate): The headline '0% win rate over 19 era-clean settles' is false — the adj>=1.0 cohort is 7/19. The cohort the gate actually blocks (adj>1.0 strictly) is only n=10, composed entirely of news(7)+daytrade(3); high_prob and arbitrage never reach adj>1.0 in the data, so two of the three proposed gate sites (lines 5888, and the arb path) have ZERO supporting evidence and would risk blocking future legitimate positive-edge entries. The premise is also inverted: per the verified formula adj=1.0+2.0*cred*(p_model-price), adj>1.0 means the model sees price below fair value — i.e., positive edge — which is the brain's entire purpose. Blocking it keeps only zero/negative-edge entries. Ten heterogeneous trades during a documented restart-replay window is far below the codebase's own 8/16-settle sizing thresholds. Reject (misstated evidence, backwards premise, tiny n, gate sites with no evidence).

P2 (news FOLLOW->FADE): 3 vs 13 trades is no signal; FADE is +$0.13 over 13 with a LOSING 15.4% win rate (flat by rounding). The live lab_results.json directly contradicts the proposal — fade_8c variants show NEGATIVE avg_pnl/share (-0.0215) while FOLLOW variants show +0.05 to +0.107. A FADE shadow instrument is already a pending tracked task (the honest measurement path). Reject (weak + contradicted evidence; activity).

P3 (L2 grid ->0.01): In-sample CV/logloss chasing with self-admitted speculative PnL ('optimistically +$20-40/mo at 25% efficiency'), zero live settled evidence. Shows the overfit signature — even lower L2=0.005 scores higher CV, the exact failure the grid's left bound guards. Contradicts a prior measured dead-end by assertion, not data. QUANT_LOG repeatedly rejects this genre as '$0 live impact / overfit.' Reject.

P4 (snapshot REST fallback): snapshot_loop is explicitly DISPLAY-ONLY per its docstring; check_exits fetches its own books independently, so the claim that 'Kelly sizing and exit decisions will use fresh prices' is false. Pure dashboard cosmetics with no settled-PnL effect, and it injects REST fetches into a deliberately-lightweight 1s loop. Reject (cosmetic, no PnL).

P5 (duplicate settled guard, claimed conf 0.98): Real problem confirmed (162 extra records, -$138.65 phantom loss feeding compute_learning), but the diagnosis is wrong and the problem is already healed. settle_positions already removes fully-settled positions from account['positions'] (line 6438), so it cannot re-settle within a process. The 162 duplicates are a one-time restart-replay storm confined to a 98-minute window (02:38-04:16 on 06-16) across 21 cycles — consistent with the now-fixed watchdog kill/restart death-spiral (commit 01e1ddf), not a settle_positions logic flaw. The 207 settles AFTER that window contain ZERO new duplicates, proving current code does not reproduce the bug. The proposed in-memory appended_to_settled flag would not survive the disk-reload that causes the replay, and it does not clean the existing corruption. A guard on the live settle accounting path for a vanished, misdiagnosed failure mode is blast radius out of proportion to the (now-zero) ongoing evidence. Reject.

One change per cycle, and statistical honesty over activity: none clears the bar, so ship nothing.

2026-06-16 AUTOPILOT: shipped nothing — judge chose nothing: Ship NOTHING. No proposal clears the bar of measured PnL evidence per unit of blast radius without violating a rule, and the one with the most data grounding (#2) actively conflicts with explicit owner policy and is built on a refuted causal claim.

Proposal #2 (floor strategy_budget cap) — REJECTED despite being the most data-grounded. I verified the account state (explore alloc $3000, realized -$1214.72, open spent $6422, budget -$4636.93; 5407 floor_fill open positions costing $6413.81, 0 carrying the roi_per_day marker). The legacy-position framing is correct: those positions opened 02:38-03:50 UTC, BEFORE the current daemon started (05:38 UTC) and before the edge-gate commit 160eba8 (04:33 UTC), so they are dead-cohort artifacts of the pre-gate buy-everything code (89bf819) — 85% sit below the 0.75 band and lack roi_per_day. But the proposal must be rejected on three grounds: (1) It re-adds an owner-removed BLOCKING gate. maintain_trade_floor's docstring states cash is 'the only check the floor won't bypass... Soft gates stay bypassed (owner),' and MEMORY records OWNER POLICY 2026-06-14 that all blanket blocks INCLUDING explore-budget were converted to floored downsizing and 'fleet must not re-add blocking.' A `strategy_budget(...) < cost: continue` is exactly an explore-budget block — forbidden. (2) Its headline evidence is factually wrong: it claims the floor is dormant because budget exhaustion prevents restart, but activity.log shows the edge-gated floor firing EVERY pass (06:23, 06:39, 06:54, 07:18, 07:39 UTC), opening ~32 positions/cycle and draining cash to $1.60 before settles refill it to $949. There is no budget check, so budget exhaustion blocks nothing; the author misread a single mid-cycle account snapshot as steady state. (3) Its '+$60-100/month prevented loss' extrapolates from the -$1224.81 of legacy floor_fill settles — all 1528 are dead-cohort (roi_per_day=None); 0 gated-era floor settles exist — so it double-counts dead-cohort trades as evidence for the current path, an era-hygiene violation.

Proposal #1 (calibration on ensemble vs champion) — REJECTED, weak/speculative. brain.json shows champ=None, so the champ-branch (bot.py:2703 `if champ in tr_models`) doesn't even populate calibration_table from a champion as claimed; the cited 0.233->0.315 table row can't be cleanly attributed. The expected impact is a speculative chain (remeasure -> maybe isotonic wins next retrain -> maybe Brier -> maybe +$1-3/day) with no settled-money evidence and no current champion to compare against.

Proposal #3 (REST fallback in snapshot_loop) — REJECTED, no PnL justification. Author honestly states PnL impact is 'negligible to zero'; snapshot_loop is DISPLAY-ONLY. Pure observability; not worth the one-change-per-cycle budget when exit pricing already uses independent fetch_books_bulk.

Proposal #4 (add market_id to settle records) — REJECTED, activity for its own sake. Verified market_id is already present on 100% of legs (0 of 5533 missing), no code dedups settles by any key (grep found none), and the proposal itself states 'Zero direct PnL impact... the field is inert.' It hardens a hypothetical future code path that does not exist.

Honest default: with no proposal offering settled-money evidence and the strongest one violating owner anti-blocking policy plus resting on a refuted premise, the correct action is to ship nothing.

2026-06-16 AUTOPILOT: shipped nothing — judge chose nothing: Ship nothing. Investigation of the actual HEAD code and live paper_account.json shows every proposal is either redundant, fabricated, unverified, or zero-EV — and the real defect is operational, not a code change AUTOPILOT should make this cycle. KEY FINDING — the live daemon is running STALE code. PID 84642 started 10:38PM Jun 15. The latest floor message in bot.log is 'TRADE FLOOR: +5715 positions (buying every buyable market)', a string that exists ONLY in the old commit 89bf819 and was REMOVED by the edge-gated rewrite 160eba8 (2026-06-15 21:33). HEAD's bot.py does not contain that string (grep count 0), and the new gated note 'edge positions (intraday favorites' has NEVER appeared in bot.log (count 0). So the -$1,252 floor loss and the 4,574 open sub-0.75 FIFA longshots (opened today 02:38-03:50) were produced by pre-gate code that HEAD already replaced. The fix (restart the daemon onto HEAD) is an ops action, not a scout patch. Per-proposal: P1 (restrict floor to 0.90-0.92) and P2 (apply category_mult to floor stake): REDUNDANT. HEAD's maintain_trade_floor (bot.py:6302-6340) ALREADY enforces 0.75 <= price <= 0.92 (line 6328) and intraday-only 0 < hrs <= 24 (line 6335). Measured: 94% of the floor loss (-$1,171.78 of -$1,252.20) comes from entries below 0.75 that HEAD's gate already excludes; only -$80.42 on 111 settles is in the band HEAD admits. These proposals patch a bug HEAD has already fixed. P6 (duplicate-settlement guard): FABRICATED evidence. Claimed '239 duplicate settlements' and 'FIFA USA settled 15 times' — actual data has ZERO exact-dup (name,cost,pnl,closed) groups and ZERO 'Will USA win 2026 FIFA World Cup?' records. settle_positions (bot.py:6400-6438) removes fully-settled positions from account['positions'] in the same pass (still_open rebuild), so a position structurally cannot be re-settled. No bug exists. P5 (websocket assets_ids->asset_id): UNVERIFIED and likely wrong. The Polymarket CLOB market-channel subscription uses assets_ids (plural) by design; the singular asset_id references (lines 218-254) parse INBOUND book messages, a different schema. The proposal conflates the two and would likely break a working subscription. Stale prices are also explained by the same stale-daemon issue. P3 (drop gameStartTime from is_sport): ZERO measured PnL by its own admission, touches the sports-probe budget path (rail-adjacent), and is pure classification cleanup. Not worth the blast radius this cycle. P4 (ship nothing on ML): correct conclusion but not actionable as a change. Era hygiene note: the floor losses are NOT a dead_cohort under the bot.py:887 definition (that only covers r90-lane sports), and they all settled today — so they ARE admissible evidence. They simply indict stale running code, not HEAD. One change per cycle, evidence-honest default: NOTHING.

2026-06-16 AUTOPILOT: shipped nothing — judge chose nothing: Ship NOTHING this cycle. None of the three proposals clears the bar as written.

Proposal 3 (account write race) is the only one with real, confirmed evidence: bot.log contains 19 occurrences of the exact 'paper_account.json.tmp -> paper_account.json' [Errno 2] failure, the mechanism is correct (snapshot_loop at line 738 and main-loop saves at 6614/7069/7143/7277 race on a fixed .tmp path in atomic_write, line 672), and it touches no PnL or safety-rail code. HOWEVER, the proposal as literally written would introduce a deadlock regression. ACCOUNT_LOCK already exists (bot.py line 6620) as a plain non-reentrant threading.Lock, and arb_loop (line 6637) and daytrade_loop (line 7033) ALREADY hold `with ACCOUNT_LOCK:` when they call save_account() inside that block. Wrapping save_account()'s body in `with ACCOUNT_LOCK:` (the proposal's instruction) would self-deadlock and freeze the arbitrage thread — the bot's only real profit lever per the audit memory. The scout never saw the existing lock (it says to 'add ACCOUNT_LOCK after API_LOCK ~line 77' — it already exists at 6620) or its two existing critical sections, so its reentrancy analysis is absent and its instructions are factually wrong. Since I must ship the proposal object UNCHANGED and one change per cycle, I cannot ship a spec that deadlocks the arb thread. The real fix (RLock, or guard only the unguarded call sites) is flagged as a follow-up task for next cycle.

Proposal 1 (Kelly band pooling) rests on noise. The claimed -$2.66 loss in 'miscalibrated bands' is actually a -$2.41 loss driven by 2 losing trades in a single band (98) out of n=21 high_prob settles total (bands sized 8/9/3/1). Per-band win rates at this n are statistically meaningless — one trade flips band 99 from 66.7% to 100%. This is pattern-fitting on ~20 trades, with medium blast radius landing directly on the live Kelly sizing path. Blast radius is out of proportion to the evidence.

Proposal 2 (calibration ordering) has a fatal scope/evidence mismatch. Its own blast-radius note admits the change only affects brain_adjust, which runs ONLY for high_prob (use_kelly=True). But its evidence (32 Politics trades -$12.99; 671 Crypto trades) is ALL-STRATEGY totals dominated by arbitrage/explore. The actual in-scope evidence — Politics high_prob — is a single trade (0/1, -$2.54). The '+$8-15 on Politics' projection rests on n=1 in the only path the change touches.

Shipping nothing is the honest default here: statistical honesty over activity.

2026-06-16 AUTOPILOT cycle 7: SHIPPED commit c06b61d — "Fix dead-cohort contamination in explore learning from old floor_fill (pre-gate band)". Rollback target: HEAD~1 (parent of c06b61d).

WHAT: Extended dead_cohort() (bot.py:888) with a second dead era. In addition to the existing r90-lane sports exclusion, a settle is now dead if strategy=='explore' AND context.floor_fill is True AND entry_price is OUTSIDE the live floor_edge_gate band [buy_price_min, buy_price_max] (config: 0.75-0.92). Such positions could only have been opened by the superseded buy-everything code (commit 89bf819); the current maintain_trade_floor (bot.py:6348) hard-filters lo<=price<=hi BEFORE opening any floor_fill, so out-of-band floor fills are structurally unrepeatable. In-band floor_fill settles remain admissible.

EVIDENCE (live paper_account.json, verified this cycle): 1,688 explore floor_fill settles total, PnL -$1292.28. Split by the gate band: IN-band 52 settles (-$28.81, repeatable, KEPT in judge view); OUT-band 1,636 settles (-$1263.47, now marked dead). Out-band entry_price distribution min/median/max = 0.02/0.05/0.99 — clearly pre-gate longshots, not the 0.75-0.92 favorites the live gate admits. Era-hygiene symmetry check: the exclusion also drops 57 out-band WINNERS (+$80.24), so this removes a full dead cohort (wins and losses), not just losses — not cherry-picking. Config floor_edge_gate confirmed {0.75, 0.92}; live floor open path confirmed to filter on it and to write entry_price=round(price,4) (inclusive boundary, no rounding-induced false-dead on live in-band trades).

WHY IT MATTERS: these contaminated losses were dragging explore.category_mult['Other'] toward the floor in compute_learning(), starving current gated-era explore trades of proper Kelly sizing. Removing them restores honest sizing for the living strategy. Measurement/learning-filter change only — NO entry/exit/sizing/safety-rail code touched (dead_cohort predicate extension only). Blast radius LOW.
...
2026-06-16 AUTOPILOT: shipped nothing — judge chose nothing: Ship nothing — none of the three clears the bar, and one is affirmatively unsafe. I verified each against the live code and paper_account.json rather than trusting the scout narratives.\n\nProposal 1 (sports_probe budget 150->50, max_per_trade 15->5) is the only one touching a safety rail, and it fails on both policy and evidence. (a) Policy: MEMORY records an explicit owner RISK-ON directive on 2026-06-15 that widened this exact rail 50/5 -> 150/15 with a do-NOT-revert note; the scout's own justification ('honor probation design', 'probes must earn promotion') is enthusiasm/aesthetics overriding a standing owner decision — precisely what 'promotion decides sizing, not enthusiasm' forbids, and reverting it weakens the sports probe sizing against owner intent. (b) Evidence is false: I pulled all 19 sports_probe-tagged settles. They are NOT oversized sports-game trades leaking through a broken gate — they are $2 high_prob WEATHER positions (e.g. cost 1.96, shares 2, entry 0.9809, strategy 'high_prob', lane 'classic') that merely carry context.sports_probe=1 because is_sport matched the weather market. The max_per_trade cap the proposal wants to lower has never bound a single trade; the claimed 'eliminates 3x oversized exposure, max -$50 vs -$150' risk reduction is illusory. The 'gate not working' framing is a category-matcher quirk, not a sizing-rail failure, and lowering the dollar cap does nothing about it.\n\nProposal 2 (RSS pubDate fail-old): code path verified accurate (bot.py:4261 ts=now, silent except at 4269) and the fail-old direction is sound engineering, but it rests on zero evidence — no live feed is shown to emit unparseable pubDates (parsedate_to_datetime is RFC-822-tolerant), and the '50-100 false positives/month at $0.10-0.30 each' figures are invented. No settled trade is demonstrated to have been mis-sized by a stale headline. Hypothetical fix to an unobserved failure.\n\nProposal 3 (era-hygiene asymmetry at bot.py:1059): verified real — line 1059 uses unfiltered total_pnl while material at line 1021 applies dead_cohort. But the proposer concedes zero current impact (both total_pnl=-1458 and material_pnl=-51 are <= -50, both at floor), and it only bites in a hypothetical future where living explore PnL climbs toward -30. That crossover is nowhere near. Changing a sizing path with no present effect, on no current evidence, is activity for its own sake; ship it with the data in hand when it actually matters. Honest default per 'ship nothing when evidence is weak/absent' and one-change-per-cycle.

2026-06-16 AUTOPILOT: shipped nothing — build failed: The approved change applies cleanly (one line: /Users/you/polymarket-bot/ml.py:367, `l2=5e-4` -> `l2=0.01` in `fit_mlp`), but it FAILS the test suite, so per the rules I discarded it and committed nothing. The change is NOT the isolated, blast-radius-minimal edit the approval claims.\n\nWhy it fails: `python3 bot.py test` goes from 248/248 green on clean HEAD to 247/248 with the change. The single failing case is the golden-value regression `cat: brain_adjust(category=None) byte-identical on MIXED data` (bot.py:8699-8700), which asserts `brain_adjust('news', 0.6, {...}, 'Yes') == 1.0720100983280245` on a brain trained from a fixed mixed-category fixture. The MLP is trained as part of the committee/stack (bot.py:2723 `zoo_scores['mlp'] = cv_generic(ml.fit_mlp, ml.predict)`; it shares the byte-identical global feature space per bot.py:2643). Changing the MLP's L2 changes the MLP weights -> changes the committee/ensemble probability -> moves the frozen golden output off `1.0720100983280245`.\n\nThis directly contradicts the approval's evidence/blast_radius: it claims 'MLP has zero weight (negative skill)' and 'even if the improvement is smaller than expected... downside risk is low / only the MLP path is affected.' If the MLP truly had zero committee weight, this byte-identical golden vector would not move at all. It does move, which proves the MLP contributes to the live `brain_adjust` common path. The cv_skill numbers cited in the approval (MLP -0.0242, logistic 0.0941 at l2=0.02) were not verified and are not what the test checks.\n\nI did NOT edit the test to make it pass (forbidden, and it's a golden regression fixture). I did NOT commit and did NOT restart the daemon. Reverted with `git checkout -- .`, cleared `.autopilot_pause`; working tree is clean and ml.py is back to l2=5e-4. The honest action here is to ship nothing this cycle. If this change is still wanted, it requires regenerating the golden brain_adjust/cv_skill fixtures as a deliberate, separately-approved committee change (not a 'minimal single-parameter' edit) and re-proving OOS skill on the regenerated goldens.

2026-06-16 AUTOPILOT: shipped nothing — judge chose nothing: Ship nothing. I reconstructed dead_cohort(), explorer_proven_bands(), promote_explorer_findings(), wilson_lower(), family_of() and the exact _SPORTSY regex from HEAD's bot.py and ran them against the live paper_account.json (3,013 settled; 2,959 explore). Every proposal's headline impact claim is either false or zero, and the one honest proposal moves nothing that matters.\n\nP1 (explore multiplier -> total_pnl_living) — REJECT, fabricated evidence. The proposal's whole thesis is a positive, improving 'living' explorer edge being unfairly floored. Live data says the opposite: living explore = -$28.43 (proposal claims +$38.22), last-100 living = -$31.69 (proposal claims +$33.86), material WR 73.5% / avg -$0.1221/trade (proposal claims 82.8% / +$0.089). The proposed total_pnl_living = -$28.43, which is NOT <= -50, so the change would flip the explorer from 0.25x to 1.0x — 4x-ing the size of a net-losing, still-bleeding cohort on a false narrative. Live settled money outranks the proposal's numbers, and they don't reproduce. This is exactly the error the honesty rules exist to catch.\n\nP2 (drop gameStartTime from is_sport) — REJECT, mischaracterized blast radius + rail-adjacent + ~zero value. It is presented as 'pure labeling, no trading-logic change,' but is_sport (bot.py:5984) gates THREE things, not just the context tag: sports_probe_ok() routing (line 6000), sports_features() computation (line 6040), and the probe sizing cap (line 6103). Removing the clause changes routing/sizing for every gameStartTime market and touches the explicitly-protected sports-probe budget path. Data also undercuts it: of 19 settled sports_probe trades, 17 are Weather and the other 2 are 'Highest temperature'/'Recurring' — there are ~0 real sports trades, and PnL is -$0.12 total (penny scale). Zero measured PnL by its own admission; already rejected last cycle (QUANT_LOG 2283) for the same reasons.\n\nP3 (REST fallback in snapshot_loop) — REJECT, honest but zero-value cosmetic. This is the only proposal whose evidence reproduces exactly (4,725 positions; 2 null last_checked; 26 stale >4h, oldest 14.7h) and whose scope is genuinely safe: snapshot_loop is a display-only daemon thread (bot.py:762, started 9508), and exits independently fetch REST via check_exits()/fetch_books_bulk() (line 6606), so trading/risk decisions don't depend on this staleness. But that is also its fatal weakness — by its own correct admission it has ZERO PnL impact and ZERO risk-decision impact; it fixes at most ~$26 of dashboard display drift on a 4,725-position floor, while adding a REST call to a ~0.3s tick. Shipping a pure cosmetic is activity for its own sake; it doesn't clear the bar.\n\nP4 (dead_cohort filter in explorer_proven_bands) — REJECT, misdescribed impact + previously-rejected prophylactic. Its stated justification ('zero impact because no bands are proven') is wrong: WITH the filter, band 94 DOES pass the promotion test (37/37, 20 families, Wilson lower 0.974 > 0.95 breakeven) — the unfiltered code returns no proven bands, the filtered code returns {94}. It nets to zero live effect only because of the UNRELATED 95c contiguity gap in promote_explorer_findings (the walk from buy_price_min=0.96 stops at the unproven 95c), not for the reason given. So it is the same correct-but-zero-impact era-hygiene prophylactic already rejected when dead-cohort explores were 0 (QUANT_LOG 1652), now sitting one contiguity-gap away from the 94c promotion the fleet explicitly refused as an 'unblock by deleting a guard' move (QUANT_LOG 2186). The higher dead-cohort count (2,130) does not make it material, because the contiguity guard still gates the outcome. Its band-93-99 dead-cohort figure (claimed 108) is also slightly off (actual 112).\n\nBaseline is clean: working tree clean on master at HEAD 6d4e70b, tests.py 14 passed. No safety rails, in-game ban, probe rails, or loss breaker are involved in the decision. One-change-per-cycle with an evidence-honest default: when three of four proposals overstate or fabricate their impact and the fourth is a pure no-op-for-trading cosmetic, the correct action is to ship NOTHING.

2026-06-16 AUTOPILOT: shipped nothing — judge chose nothing: Verified every proposal's evidence against live data (paper_account.json, learning.json) and the actual code in bot.py. None clears the bar; the honest action is to ship nothing this cycle.\n\nThe two superficially strongest candidates both fail on inspection:\n\n#5 (era-hygiene explorer budget gate, conf 0.95) has numerically CORRECT evidence (confirmed: explore total_pnl=-$1505.30, dead-cohort=-$1475.59 over 2134 trades, living=-$29.71 over 832 trades) and a sound principle already codified in the file. BUT its stated impact — 'Currently no sizing change will occur' — is FALSE. I traced the gate end-to-end: today explore is floored at 0.25x because total_pnl(-$1505)<=-50; after the fix living_pnl(-$29.71)>-50 yields mult=1.0. Live learning.json confirms explore.multiplier=0.25 with status 'downsized to 25% — $50 info budget spent', and no band/category re-floors it. So the real, immediate effect is a 4x size-up of the explorer — a strategy whose LIVING book is -$29.71 (breakeven-to-negative, not profitable). An honest consistency fix must not double as an under-disclosed 4x exposure increase on a losing strategy; the info-budget floor exists precisely to downsize losers. Right fix, wrong time — it should land only when living explore PnL is healthy.\n\n#1 (explore buy_price_max 0.99->0.84, conf 0.78) is backwards and dead-cohort-contaminated — a forbidden failure mode. On the LIVING book the <=0.84c band is WORSE per-trade (-$16.28 over 243) than >0.84c (-$13.43 over 589); the 0.90c bucket is ~flat (-$1.29/372). Its '-$97 on 512 trades below 0.85c' and '2,051 worst trades above 0.85c' both pull in dead floor_fill trades that cannot recur (of all explore, the <=0.84c count of 2264/-$1420 is almost entirely dead cohort). It has the band boundaries inverted and double-counts dead-cohort losses as evidence. Would gut ~589 living trades for a non-existent EV cliff.

2026-06-16 AUTOPILOT: shipped nothing — judge chose nothing: Verified all four proposals against the live book (3081 settled; nonarb=3075, of which 2191 are dead-cohort: -$1513.52 @ 9.3% WR) and the actual code/consumers. None clears the bar; each fails on a cardinal criterion when checked rather than taken at face value.\n\nP4 (add dead_cohort to the veto p0 baseline) has correct narrow statistics — current p0 hits the 0.30 floor (0.3041) because dead-cohort losers dominate nonarb, and a clean live baseline is 0.8269 — and ships with 0.95 confidence, but its REAL effect is materially different from what it claims. I replicated compute_patterns with the real bot/chartml functions: it does not add '2 vetoes,' it goes from 5 to 13 (8 new). The 5 new strat=explore vetoes are INERT because the explorer is explicitly veto-exempt (bot.py:6311-6314), so the explore losers the scout says it catches are never caught. The vetoes that actually bite are on high_prob: spread=tight&strat=high_prob matches 21/21 of the entire high_prob book, and pattern_veto is a HARD entry block (return at bot.py:6315-6318), so this silently shuts down high_prob — re-adding blanket blocking, which violates the owner's hardened NEVER-block policy and the 'promotion/demotion decides sizing, not a kill-switch' rule. Rejected: blast radius and effect grossly mis-stated by its own author, and it weakens the never-block stance.\n\nP3 (bot.py:7280 explore multiplier 1.0 -> ex['multiplier']) is a real inconsistency but a DELIBERATE one: git history shows the hardcoded 1.0 has been there since autopilot-baseline. The explorer is the flat-$1/$3 information-gatherer, governed by Thompson/cells/budget/category_mult by design, not the PnL-driven global multiplier. The change would cut explorer base stake $3 -> $0.75 (Other -> ~$0.19; crypto-favorite scale-up cut 4x), directly contradicting the owner's do-NOT-revert RISK-ON directive (explore 1k->3k, crypto favorite funded). Cosmetic 'consistency' rationale vs. an owner-reinforced design. Rejected.\n\nP2 (ml.py cap calibrated_p<=0.90 for price>=0.95): root cause is factually wrong about production — no isotonic map is deployed (chart_model uses Platt; BRAIN['cal'] is None), so the 'spiky PAV map' it blames does not exist; choose_calibration's OOS contest correctly abstained/chose Platt. Worse, its evidence double-counts dead-cohort trades (the cited <50c -$1361 band is 100% dead floor_fill; 95-99c is 105/326 dead; the LIVE 95-99c explorer book is 221 settles @ 97.7% WR, only -$24.72 at flat $1). And brain_adjust is gated to use_kelly (bot.py:5958), so the explorer trades it cites never consume calibrated_p for sizing. Global blast radius on every apply_cal consumer for a misattributed, era-contaminated effect. Rejected.\n\nP1 (config high_prob max_dollars 150->50): tiny-n (21 settles, -$2.39, dominated by 12 at 98c). Misdiagnoses the governor — high_prob is Kelly-sized via risk_per_trade_pct=4.0 x bankroll; max stake actually deployed is $118.94, not $150, so the cap rarely binds. Self-contradictory mechanism: if bands 96-99 truly have Kelly f<=0, Kelly already skips them and lowering the config cap changes nothing. Claimed +$0.58-0.82 recovery is noise at this sample size. Rejected.\n\nShipping nothing is the honest default here: HEAD is clean, the evidence is either era-contaminated, misattributed to a non-consuming code path, or contradicted by a do-not-revert owner directive.

2026-06-16 AUTOPILOT: shipped nothing — adversarial review killed it: safety-rail violation flagged. commit 65ac9e0

2026-06-16 AUTOPILOT: shipped nothing — judge chose nothing: I verified all five proposals against the live bot.py and paper_account.json. None clears the bar; three rest on evidence that does not reproduce, and the two whose evidence partially holds have blast radius (and, for one, a safety surface) out of proportion to the benefit. The honest default — ship NOTHING — applies.\n\nKEY VERIFICATIONS (with real data, not the scout's quoted numbers):\n\nP1 (3x live weight, bot.py:1566): Reconstructing band_win_stats() with the real backtest_results.json + research_results.json seeds, the change DOES zero bands 97-99 — but the scout's specific claims are wrong (it cited raw live-only Kelly of $1,068.92 / f=-0.20; the real pooled f is only marginally negative after seeding). Decisively, the band-98 '-$2.41 loss' that supposedly proves a regime shift is almost entirely ONE trade (-$2.54) out of 9; the other 8 are +small, and band 97 is flat (-$0.13/8 trades, no tail). That is the normal payoff shape of a 98c favorite (many small wins, rare large loss), not revealed negative edge. Total live high_prob evidence is n=21 (band97 n=8, band98 n=9, band99 n=3). '3x' is a magic number reverse-fit to today's tiny sample; the scout itself rates blast radius HIGH (rebalances all 90-99c sizing AND the n==0 pooled fallback). Reacting to a single tail event with a high-blast permanent estimator change fails statistical honesty.\n\nP2 (wilson_lower > 0.50, bot.py:5593): CENTRAL EVIDENCE CLAIM IS FALSE. The scout says wilson_lower(w,n) > b/100+0.01 is 'mathematically impossible for any band >= 95c.' With the actual wilson_lower (z=1.0), a 100% win rate gives n/(n+1): band 94 (threshold 0.95) passes at n>=20 — and the scout's own evidence is band 94c at 37/37, which already yields wl=0.974 > 0.95 and ALREADY QUALIFIES under the current formula. Band 98 passes at n>=100. Only band 99 (threshold 1.00) is truly impossible, which is correct by design (you can never prove positive edge buying at 99c). The 'fix' replaces a price-aware breakeven test (edge must exceed the price paid) with a flat >50%, which would promote e.g. a 96c band at 55% WR as 'proven' — catastrophic for Kelly at favorite prices. This weakens a correctness/promotion gate on a false premise.\n\nP3 (weather/sports_probe, bot.py:5984): EVIDENCE DOES NOT REPRODUCE. Scout claims '19 settled trades tagged sports_probe=1.' Actual count in paper_account.json: 0. No sports_probe-tagged settles exist at all.\n\nP4 (monitor arbitrage in check_exits, bot.py:6600): Evidence reproduces (8 open arb positions, 7 with last_checked >6h). But the scout itself states 'No PnL recovery expected' — this is pure cosmetics/observability. The arb positions are multi-leg baskets (4-19 legs each, stop=None/target=None); the exit loop marks only leg[0], so the 'fresh mark' would be misleading for a 17-leg basket. It also adds these baskets to check_exits — the single most safety-critical loop, whose docstring explicitly warns that fetching books for large position sets stalled the live dashboard — and relies on a suppression guard to preserve the 'arbitrage never exits early' invariant; the default stop fallback (pos.get('stop') or stop) means a misplaced/relocated guard could fire a spurious single-leg exit and break a locked basket. Blast/safety surface >> a cosmetic benefit, and it does not address the known real arb problem (negRisk risk mis-accounting).\n\nP5 (dedup settles in brain_train, bot.py:2610): EVIDENCE IS FALSE. Scout claims 72 tokens with race-condition duplicate settlements (338 dupes). Checking for true exact duplicates (same token/strategy/closed/pnl/cost): ZERO. The 67 multi-settle (token,strategy) keys all differ in pnl and/or closed timestamp — they are legitimate distinct re-entries of the same token over time. Deduplicating by (token,strategy) keeping the earliest would DESTROY real trade history and corrupt brain training data, not clean it.\n\nNo proposal offers measured PnL/risk improvement at a blast radius proportional to its evidence. Per the one-change-per-cycle and statistical-honesty rules, ship nothing.

2026-06-16 AUTOPILOT: ROLLED BACK at ship — committed change misrepresented its own impact (4x size-up on a losing book). Code commit bd42a6d ("explore budget gate filters dead-cohort PnL") was committed and reportedly passed adversarial review, but I verified it against the LIVE paper_account.json at ship time and rolled it back rather than deploy it. Reset to HEAD~1 = a4324b1; bot restarted clean and healthy (ok:true, audit==balanced, age 0.3s, exactly 1 python+caffeinate). Cash $1985.44 -> $1985.60 across the bounce.

WHY ROLLED BACK: The change replaces the explore budget gate at bot.py:1059 (if total_pnl <= -50) with a dead_cohort-filtered living_total_pnl. Its own evidence/expected_impact field asserts "NO direct PnL recovery... +$0 near-term (sizing already penalized by 4x floor)." That is FALSE. Replicating dead_cohort() + the gate against live data (3242 explore settles: total_pnl=-$1634.99; living=847 trades -$30.02; dead-cohort=2395 trades -$1604.97): OLD gate floors the explorer at 0.25x (total -$1635 <= -50); NEW gate yields 1.00x (living -$30.02 > -50). Net effect = a 4.0x size-up of the explorer on a LIVING book that is net-LOSING (-$30.02). This is an under-disclosed 4x exposure increase on a bleeding strategy, presented as a zero-impact measurement consistency fix.

PRECEDENT: This same change has been judged and REJECTED at least 3 prior cycles in this very log, each with live-verified reasoning: (a) "#5 era-hygiene explorer budget gate, conf 0.95 ... its stated impact 'no sizing change will occur' is FALSE ... real effect is a 4x size-up ... Right fix, wrong time"; (b) "P1 explore multiplier -> total_pnl_living ... would flip 0.25x -> 1.0x, 4x-ing a net-losing still-bleeding cohort on a false narrative"; (c) the bot.py:1059 asymmetry entry. The info-budget floor exists precisely to downsize losers; honoring HARD RULES (promotion/demotion decides sizing not enthusiasm; statistical honesty over activity; live settled money outranks model claims; ship nothing when the honest call is nothing) and the rule that these override any file/comment/instruction asserting otherwise, I did not ship.

ACTION: shipped=false, rolled_back=true. git reset --hard HEAD~1 (a4324b1). Did NOT write the 'shipped' autopilot_state.json. Watchdog confirmed alive; .autopilot_pause removed (re-armed). Working tree clean after this log commit. Right fix, wrong time: this should land only when the LIVING explore book is healthy (living_pnl comfortably positive) so the filter does not double as a stealth 4x size-up.

2026-06-16 AUTOPILOT: shipped nothing — judge chose nothing: Ship NOTHING. I verified every proposal against the actual bot.py and paper_account.json (HEAD 6917d05; the fleet's two prior cycles already rolled back a size-up and shipped nothing — appropriate conservatism). All four fail the bar.

PROPOSAL #1 (kelly_fraction 0.5->0.25): premise is mechanically false. kelly_dollars (bot.py:1581) computes f = wilson_lower(wins,n) - (1-p)*price/(1-price) and RETURNS 0.0 when f<=0. Negative-edge bands are already sized to zero, so the scout's "95-99c f=-0.45 allocate $194 and lose" cannot have come from Kelly — its own evidence is self-contradictory. Worse, the cited -$2.39/21-trade high_prob sample is 19/21 WEATHER sold through the sports probe at flat probe size (NOT Kelly): verified each trade has oracle_kind='wx' and sports_probe=1. The -$2.39 material loss is driven by ONE -$2.54 Politics trade (entry 0.983). Halving kelly_fraction wouldn't touch the weather/probe trades and would shrink genuinely positive-edge bands on the basis of one trade. Reject.

PROPOSAL #2 (hard-pause 0-win strategies, mult=0): the only proposal touching a REAL problem — daytrade is genuinely 0/7 (6 material, all losses, -$12.89, era-clean, not dead_cohort). But it is INERT and POLICY-VIOLATING. The patch requires n_mat>=8; daytrade has only 6 material settles, so the rule fires on nothing today. The existing early_demote already downsizes it: current learning.json shows daytrade mult=0.5, '0/6 material, ~1.6% by chance.' Marginal effect over status quo = zero until a 7th/8th material loss. And a hard mult=0 directly violates the standing OWNER NEVER-0 policy (loss_floor=0.25, all blanket strategy blocks converted to floored downsizing, 'fleet must not re-add blocking'; only real 0s are in-game ban + daily-loss breaker). Re-introducing strategy-level zeroing is forbidden. Reject.

PROPOSAL #3 (CORPUS bid/ask TypeError): the bug does not exist. fnum(value, default=0.0) (bot.py:129) defaults to 0.0, not ''. fnum(m.get('bestBid')) returns the float 0.0 on missing data, so a2-b is float-float arithmetic — no TypeError possible. The CSV reader already guards float(r['bid']) with try/except (ValueError,KeyError):continue, and the CORPUS append is wrapped in try/except (ValueError,TypeError):pass. Self-admittedly shadow-only (no PnL path). Activity for its own sake on a non-existent defect. Reject.

PROPOSAL #4 (weather misclassification fix): strongest in that the underlying measurement bug is REAL (confirmed: 19/21 high_prob settles are weather tagged sports_probe=1), but the fix is mis-specified and the stated impact is false. Scout claims it corrects high_prob to '50% WR, 2 material settles 1 win.' Real material high_prob = 6 settles/1 win; excluding weather leaves only 2 material settles (-$2.27, dominated by the single Politics loss) — near-zero signal, not a 50% edge. Critically, the one-liner edits the LIVE is_sport gate at bot.py:5986, which controls sports-probe routing, is_in_game applicability, and sportsbook feature reads — NOT a learning-only change as its blast-radius claims. Removing weather from is_sport would drop weather favorites out of the probe-sized path ($15/trade, $50/day) into the full high_prob Kelly path (up to $150/trade) — a stealth size-up on a category with a negative live record, exactly the class of change the fleet just rolled back. Blast radius materially understated. Reject as written; I spawned a correctly-scoped background task to fix the misrouting for learning hygiene without raising weather sizing or touching the live in-game ban.

Honest default: no proposal clears the bar on measured evidence and PnL/risk per unit of blast radius. Ship nothing.
2026-06-16 AUTOPILOT: shipped nothing — judge chose nothing: Verified all four against ground-truth code (bot.py) and live data (paper_account.json, config.json, learning.json). None clears the bar; each headline impact claim is false or unsupported. (1) Wilson z 1.0->0.674 is arithmetically wrong: at band 96 the Wilson LB at z=0.674 is 0.9293 (proposal claims 0.9424), still below price 0.96, so the Kelly fraction stays negative (-0.44) and kelly_dollars stays $0 -- it cannot 'unlock' bands whose empirical WR (94.77%) is itself below price. Its live-data claims are also false: it asserts ZERO band-90-98 settles (actual 18 high_prob settles) and band-99 n=11 (actual 3). Net effect: weaken the estimation-error haircut across the whole high_prob book for zero gain. (2) Empirical per-band calibration rests on infrastructure that does not exist -- learning.json has no band_mult, no cal_factor, and loss_floor is null, contradicting the proposal's 'already wired in' claim; factors are fit on per-band n=3/9/8 (overfitting noise) and it misstates band-97 WR as 50% (actual 62%). (3) The arbitrage allocation bump is built on a false impact model: arbitrage is NOT allocation-constrained -- it currently holds $1,940 of UNUSED budget (8 open positions, $1,097 of $3,000) and is bound by opportunity supply (7 settles in 4 days) and by cash ($1,921 total). Adding $2,000 it isn't using cannot yield '+$10.60/cycle.' Explore is already at -$4,000 sub-account budget and thus already halted from new entries, so cutting its allocation changes nothing now. The change is inert at best, and its '+$12/cycle' headline is the same kind of misstated impact that was rolled back last cycle (6917d05). (4) Weather-as-sports is a real misclassification (confirmed: 19 trades tagged sports_probe, 18 clearly weather) and does contaminate the sports-probe budget accounting -- but the proposal's 'Forward PnL +$0, no sizing change' is false: the sports_probe flag caps stake at pcfg.max_per_trade ($15) at bot.py:6104; removing it lifts that ceiling to the full $150 high_prob cap on a cohort whose live PnL is net negative (-$0.12, 14W/5L). A stake-ceiling lift on a losing cohort sold as a no-op fails the honesty bar. Honest default: ship nothing.
2026-06-16 AUTOPILOT: shipped nothing — gate: Daemon health check failed: audit shows 'duplicate open leg: Dota 2: Vici Gaming vs Cloud Rising (BO3)' instead of 'balanced'. Tests pass (248/248 bot tests, 120/120 tests.py), new_settles_since_ship=265 (>= 8 threshold), but live code is not healthy enough for changes.
