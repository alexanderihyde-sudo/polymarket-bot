...
const discardPrompt = (note) => R([
  ...
  '4. Append ONE dated line to ~/polymarket-bot/QUANT_LOG.md: "AUTOPILOT: shipped nothing — ' + note.replace(/"/g, "'") + '", then commit ONLY that file so the tree stays clean for the next cycle: `cd ~/polymarket-bot && git add QUANT_LOG.md && git commit -q -m "AUTOPILOT log: shipped nothing"`. Then best-effort mirror to GitHub (ignore any push error): `git fetch origin master 2>&1 | tail -1; git pull --rebase --autostash origin master 2>&1 | tail -2 || git rebase --abort; git push origin master 2>&1 | tail -1 || true`.',
